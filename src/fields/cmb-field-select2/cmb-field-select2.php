...
/**
 * Class RNOC_PW_CMB2_Field_Select2
 */
class RNOC_PW_CMB2_Field_Select2
{
    ...
    {
        add_filter('cmb2_render_rnoc_pw_select', array($this, 'render_rnoc_pw_select'), 10, 5);
        add_filter('cmb2_render_rnoc_pw_multiselect', array($this, 'render_rnoc_pw_multiselect'), 10, 5);
        add_filter('cmb2_sanitize_rnoc_pw_multiselect', array($this, 'rnoc_pw_multiselect_sanitize'), 10, 4);
        add_filter('cmb2_types_esc_rnoc_pw_multiselect', array($this, 'rnoc_pw_multiselect_escaped_value'), 10, 3);
        add_filter('cmb2_repeat_table_row_types', array($this, 'rnoc_pw_multiselect_table_row_class'), 10, 1);
    }

    /**
     * Render select box field
     */
    public function render_rnoc_pw_select($field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object)
    {
        ...
        echo $field_type_object->select(array(
            'class' => 'rnoc_pw_select2 rnoc_pw_select',
     ...
     */
    public function render_rnoc_pw_multiselect($field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object)
    {
        $this->setup_admin_scripts();
        ...
        $a = $field_type_object->parse_args('rnoc_pw_multiselect', array(
            'multiple' => 'multiple',
            'style' => 'width: 99%',
            'class' => 'rnoc_pw_select2 rnoc_pw_multiselect',
            'name' => $field_type_object->_name() . '[]',
            'id' => $field_type_object->_id(),
            'desc' => $field_type_object->_desc(true),
            'options' => $this->get_rnoc_pw_multiselect_options($field_escaped_value, $field_type_object),
            'data-placeholder' => $field->args('attributes', 'placeholder') ? $field->args('attributes', 'placeholder') : $field->args('description'),
        ));
    ...
    /**
     * Return list of options for rnoc_pw_multiselect
     *
     ...
     */
    public function get_rnoc_pw_multiselect_options($field_escaped_value = array(), $field_type_object)
    {
     ...
     */
    public function rnoc_pw_multiselect_sanitize($check, $meta_value, $object_id, $field_args)
    {
     ...
     */
    public function rnoc_pw_multiselect_escaped_value($check, $meta_value, $field_args)
    {
        if (!is_array($meta_value) || !$field_args['repeatable']) {
            return $check;
        }
     ...
     */
    public function rnoc_pw_multiselect_table_row_class($check)
    {
        $check[] = 'rnoc_pw_multiselect';
    ...
    {
        $asset_path = apply_filters('rnoc_pw_cmb2_field_select2_asset_path', plugins_url('', __FILE__));
...
$rnoc_pw_cmb2_field_select2 = new RNOC_PW_CMB2_Field_Select2();
