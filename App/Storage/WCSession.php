...
class WCSession extends Base {
		...
		if ( function_exists( 'WC' ) && is_object( WC() ) && isset( WC()->session ) && ! is_null( WC()->session ) && is_object( WC()->session ) && method_exists( WC()->session, 'has_session' ) ) {
			if ( ! WC()->session->has_session() && ! defined( 'DOING_CRON' ) ) {
				if ( method_exists( WC()->session, 'set_customer_session_cookie' ) ) {
					$rnoc_varnish_check = Settings::get( 'retainful_settings', 'rnoc_varnish_check', 'no' );
	...
	function getValue( $key ) {
		...
		return function_exists( 'WC' ) && is_object( WC() ) && is_object( WC()->session ) && method_exists( WC()->session, 'get' ) ? WC()->session->get( $key ) : null;
