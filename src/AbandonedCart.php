...
class AbandonedCart
{
    ...
    function getUserSessionKey()
    {
        ...
        if (empty($session_key)) {
            ...
            $this->setSessionToken($session_key);
        }
        return $session_key;
    }
     ...
     * @param $session_key
     */
    function setSessionToken($session_key)
    {
    ...
    {
        global $wpdb;
        $query = "SELECT template.id,template.send_after_time FROM `{$this->email_templates_table}` as template WHERE template.id NOT IN (select template_id FROM `{$this->email_queue_table}` WHERE cart_id = {$cart_id}) AND template.id NOT IN (select template_id FROM `{$this->email_history_table}` WHERE abandoned_order_id = {$cart_id}) AND template.language_code = (SELECT `language_code` FROM `{$this->cart_history_table}` WHERE id={$cart_id}) AND template.is_active = '1' ORDER BY template.send_after_time ASC LIMIT 1";
    ...
    function recoverUserCart()
    {
        if (isset($_GET['retainful_cart_action']) && isset($_GET['validate'])) {
            if ($_GET['retainful_cart_action'] == 'recover' && !empty($_GET['validate'])) {
                ...
                parse_str($cart_link, $data);
                if (isset($data['url']) && isset($data['abandoned_cart_id']) && isset($data['session_id']) && isset($data['email_sent'])) {
                    ...
                    $abandoned_cart_history_results = $wpdb->get_row($wpdb->prepare($abandoned_cart_history_query, $data['abandoned_cart_id']), OBJECT);
                    $user_id = 0;
                    if (!empty($abandoned_cart_history_results)) {
                        $user_id = $abandoned_cart_history_results->customer_key;
                        $this->wc_functions->setPHPSession(RNOC_PLUGIN_PREFIX . 'recovered_cart_id', $data['abandoned_cart_id']);
                        $this->autoLoadUserCart($abandoned_cart_history_results->cart_contents, $data['abandoned_cart_id'], $data['session_id']);
