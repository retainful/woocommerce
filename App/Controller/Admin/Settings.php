...
class Settings extends BaseController {
	...
	public static function registerMenu() {
		...
		add_menu_page( 'Retainful', 'Retainful', 'manage_woocommerce', 'retainful_license', array(
			self::class,
			'retainfulLicensePage'
		), 'dashicons-controls-repeat', 56 );
		add_submenu_page( 'retainful_license', 'Connection', 'Connection', 'manage_woocommerce', 'retainful_license', array(
			self::class,
			'retainfulLicensePage'
		) );
		add_submenu_page( 'retainful_license', 'Settings', 'Settings', 'manage_woocommerce', 'retainful_settings', array(
			self::class,
			'retainfulSettingsPage'
		) );
		if ( isset( $_REQUEST['page'] ) && in_array( $_REQUEST['page'], array(
				'retainful_license',
				'retainful_settings'
			) ) ) {
	...
	public static function getPlanDetails( $response = \stdClass::class ) {
		...
		$period_end = ! empty( $response->period_end ) ? strtolower( $response->period_end ) : 'never';
