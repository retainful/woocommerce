...
class Settings {
	...
	function switchToCloudNotice() {
		if ( ! $this->isNewInstallation() ) {
			$move_to_cloud_url = admin_url( 'admin.php?page=' . $this->slug
			                                . '_license&move_to_cloud=yes' );

			return '<p style="padding: 2em;background: #ffffff;border: 1px solid #e9e9e9;box-shadow: 0 1px 1px rgba(0,0,0,.05);">'
			       . esc_html__( "Manage your abandoned carts effectively in Retainful Dashboard & get more features ",
					RNOC_TEXT_DOMAIN )
			       . '&nbsp; <a class="button-primary align-right" href="'
			       . $move_to_cloud_url . '">' . esc_html( "Switch to cloud!" )
			       . '</a>&nbsp;<a href="https://www.retainful.com/blog/abandoned-cart-solutions-cloud-based-solutions-vs-self-hosted-plugin-based-solutions" target="_blank">'
			       . __( "Learn more", RNOC_TEXT_DOMAIN ) . '</a></p>';
		}
	...
	function pluginActionLink( $plugin, $action = 'activate' ) {
		...
		$url                = sprintf( admin_url( 'plugins.php?action='
		                                          . $action
		                                          . '&plugin=%s&plugin_status=all&paged=1&s' ),
			$plugin );
	...
	function validateAppKey() {
		...
		if ( ! $is_production ) {
			wp_send_json_error( 'You can only change you App-Id and Secret key in production store!',
				500 );
		}
		$app_id       = isset( $_REQUEST['app_id'] )
			? sanitize_text_field( $_REQUEST['app_id'] ) : '';
		$secret_key   = isset( $_REQUEST['secret_key'] )
			? sanitize_text_field( $_REQUEST['secret_key'] ) : '';
		$options_data = array(
			RNOC_PLUGIN_PREFIX . 'is_retainful_connected' => '0',
		...
		if ( empty( $response ) ) {
			...
			if ( isset( $api_response['success'] ) ) {
				...
				$options_data[ RNOC_PLUGIN_PREFIX . 'is_retainful_connected' ]
					= 1;
				update_option( $slug . '_license', $options_data );
				...
				$response['error'] = __( 'Please check the entered details',
					RNOC_TEXT_DOMAIN );
			}
		}
		wp_send_json( $response );
	...
	function sanitizeBasicHtml( $html ) {
		try {
			...
			$allowed_html
				            = apply_filters( 'rnoc_sanitize_allowed_basic_html_tags',
				$allowed_html );
			$sanitized_html = wp_kses( $html, $allowed_html );

			return apply_filters( 'rnoc_sanitize_basic_html', $sanitized_html,
				$html, $allowed_html );
		} catch ( \Exception $e ) {
			return '';
		}
	...
	static function validateColor( $field, $value, array $params, array $fields
		...
		return (bool) preg_match( '/^#(([0-9a-fA-F]{2}){3}|([0-9a-fA-F]){3})$/',
			$value );
	}
	...
	function validateCouponTimer( $validator ) {
		$validator->rule( 'in', array(
			...
			. RNOC_PLUGIN_PREFIX . 'enable_position',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'enable_checkout_button',
		...
		$validator->rule( 'color', array(
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_background',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_color',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_coupon_code_color',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_coupon_timer_color',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'checkout_button_color',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'checkout_button_bg_color',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_above_cart_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_background',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_above_cart_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_color',
		...
		$validator->rule( 'basicTags', array(
			...
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_below_discount_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_display_format',
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_below_discount_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_message',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_above_cart_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_display_format',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_above_cart_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_message',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_above_cart_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'checkout_button_text',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_display_format',
			RNOC_PLUGIN_PREFIX . 'coupon_timer_top_position_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'coupon_timer_message',
		...
		if ( self::$input->has_post( RNOC_PLUGIN_PREFIX
		                             . 'enable_coupon_timer' )
		     && self::$input->post( RNOC_PLUGIN_PREFIX . 'enable_coupon_timer',
				'1' ) == 1
		) {
	...
	function validateExitIntentPopup( $validator ) {
		$validator->rule( 'in', array(
			...
			. RNOC_PLUGIN_PREFIX . 'enable_mobile_support',
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_mobile_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'enable_mobile_back_click',
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_mobile_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'enable_delay_trigger',
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_mobile_settings.*.'
			. RNOC_PLUGIN_PREFIX . 'enable_scroll_distance_trigger',
		), [ '0', '1' ] )->message( 'This field contains invalid value' );
		$validator->rule( 'in', array(
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_show_settings.show_option',
		), [
			'once_per_page',
			'every_time_on_customer_exists',
			'show_x_times_per_page',
			'once_per_session'
		] )->message( 'This field contains invalid value' );
		...
		$validator->rule( 'min', array(
			...
			. RNOC_PLUGIN_PREFIX . 'exit_intent_modal_distance',
		          ...
		          ->message( 'This field accepts only value greater than or equal to 1' );
		$validator->rule( 'in', array(
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_gdpr_compliance.*.'
			. RNOC_PLUGIN_PREFIX . 'gdpr_compliance_checkbox_settings',
		), [
			'no_need_gdpr',
			'dont_show_checkbox',
			'show_and_check_checkbox',
			'show_checkbox'
		] )->message( 'This field contains invalid value' );
		$validator->rule( 'integer', array(
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_mobile_settings.*.'
		...
		$validator->rule( 'color', array(
			...
			. RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_button_color',
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_design.*.'
			. RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_button_bg_color',
		) )->message( 'This field accepts only hex color code' );
		$validator->rule( 'basicTags', array(
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_gdpr_compliance.*.'
			. RNOC_PLUGIN_PREFIX . 'gdpr_compliance_checkbox_message',
		...
		$validator->rule( 'regex', array(
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_design.*.'
			. RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_button_width',
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_design.*.'
			. RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_button_height',
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_design.*.'
			. RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_email_width',
			RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_design.*.'
			. RNOC_PLUGIN_PREFIX . 'exit_intent_popup_form_email_height',
		), '/^[a-z0-9%]+$/' )
		          ->message( 'This field should only accepts numbers, lowercase alphabets and percentage symbol' );
		if ( ! $validator->validate() ) {
			wp_send_json_error( $validator->errors() );
		}
	}
	...
	function validateAddToCartPopup( $validator ) {
		$validator->rule( 'in', array(
			...
			RNOC_PLUGIN_PREFIX . 'modal_coupon_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'need_coupon',
		), [ '0', '1' ] )->message( 'This field contains invalid value' );
		$validator->rule( 'in', array(
		...
		), [ 'add_and_close', 'just_close' ] )
		          ->message( 'This field contains invalid value' );
		$validator->rule( 'array', array(
			RNOC_PLUGIN_PREFIX . 'modal_display_pages',
		) )->message( 'This field contains invalid value' );
		$validator->rule( 'regex', array(
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_email_field_width',
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_button_field_width',
		), '/^(\d*\.)?\d+$/' )
		          ->message( 'This field should only accepts numbers and decimals' );
		$validator->rule( 'color', array(
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_heading_color',
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_add_cart_color',
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_add_cart_bg_color',
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_add_cart_border_top_color',
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_bg_color',
			RNOC_PLUGIN_PREFIX . 'modal_design_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_add_cart_no_thanks_color',
			RNOC_PLUGIN_PREFIX . 'modal_coupon_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'modal_sub_heading_color',
		) )->message( 'This field accepts only hex color code' );
		$validator->rule( 'basicTags', array(
			...
			. RNOC_PLUGIN_PREFIX . 'gdpr_compliance_checkbox_message',
			RNOC_PLUGIN_PREFIX . 'modal_coupon_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'add_to_cart_coupon_popup_template',
			RNOC_PLUGIN_PREFIX . 'modal_coupon_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'coupon_mail_template',
		) )->message( 'Script tag and iframe tag were not allowed ' );
		$validator->rule( 'in', array(
			RNOC_PLUGIN_PREFIX . 'modal_coupon_settings.*.' . RNOC_PLUGIN_PREFIX
			. 'show_woo_coupon',
			...
			'send_via_email',
			'both',
			'auto_apply_and_redirect',
			'send_mail_auto_apply_and_redirect_cart',
			'send_mail_auto_apply_and_redirect',
			'auto_apply_and_redirect_cart'
		] )->message( 'This field contains invalid value' );
		if ( ! $validator->validate() ) {
			wp_send_json_error( $validator->errors() );
		}
	...
	function addOrderDetailMetaBoxes( $post_type ) {
		if ( 'shop_order' === $post_type ) {
			add_meta_box( 'retainful_order_meta',
				__( 'Retainful token', RNOC_TEXT_DOMAIN ),
				array( $this, 'orderMetaDetails' ), $post_type, 'side',
				'default' );
		}
	}
	...
	function orderMetaDetails() {
		...
		echo '<p>' . $this->wc_functions->getOrderMeta( $order,
				"_rnoc_user_cart_token" ) . '</p>';
	}
	...
	function savePremiumAddOnSettings() {
		...
		$post      = self::$input->post();
		$validator = new Validator( $post );
		Validator::addRule( 'float', array( __CLASS__, 'validateFloat' ),
			'must contain only numbers 0-9 and one dot' );
		Validator::addRule( 'basicTags',
			array( __CLASS__, 'validateBasicHtmlTags' ),
			'Only br, strong, span,div, p tags accepted' );
		Validator::addRule( 'color', array( __CLASS__, 'validateColor' ),
			'must contain only hex color code' );
		...
		$modal_coupon_settings
		           = self::$input->post( RNOC_PLUGIN_PREFIX
		                                 . 'modal_coupon_settings', array(),
			false );
		$exit_intent_popup_template
		           = self::$input->post( RNOC_PLUGIN_PREFIX
		                                 . 'exit_intent_popup_template', '',
			false );
		$add_to_cart_coupon_popup_template
		           = isset( $modal_coupon_settings[0][ RNOC_PLUGIN_PREFIX
		                                               . 'add_to_cart_coupon_popup_template' ] )
			? $modal_coupon_settings[0][ RNOC_PLUGIN_PREFIX
			                             . 'add_to_cart_coupon_popup_template' ]
			: '';
		$coupon_mail_template
		           = isset( $modal_coupon_settings[0][ RNOC_PLUGIN_PREFIX
		...
		$atc_gdpr_settings
			...
			array(), false );
		$add_to_cart_gdpr_message
		           = isset( $atc_gdpr_settings[0][ RNOC_PLUGIN_PREFIX
		                                           . 'gdpr_compliance_checkbox_message' ] )
			? $atc_gdpr_settings[0][ RNOC_PLUGIN_PREFIX
			                         . 'gdpr_compliance_checkbox_message' ]
			: '';
		$ei_gdpr_settings
		           = self::$input->post( RNOC_PLUGIN_PREFIX
			...
			array(), false );
		$ei_gdpr_message
		           = isset( $ei_gdpr_settings[0][ RNOC_PLUGIN_PREFIX
		                                          . 'gdpr_compliance_checkbox_message' ] )
			? $ei_gdpr_settings[0][ RNOC_PLUGIN_PREFIX
			                        . 'gdpr_compliance_checkbox_message' ] : '';
		$data      = $this->clean( $post );
		if ( ! empty( $exit_intent_popup_template ) ) {
			$data[ RNOC_PLUGIN_PREFIX . 'exit_intent_popup_template' ]
				= $this->sanitizeBasicHtml( $exit_intent_popup_template );
		}
		if ( ! empty( $add_to_cart_coupon_popup_template ) ) {
			$data[ RNOC_PLUGIN_PREFIX
			       . 'modal_coupon_settings' ][0][ RNOC_PLUGIN_PREFIX
			                                       . 'add_to_cart_coupon_popup_template' ]
				= $this->sanitizeBasicHtml( $add_to_cart_coupon_popup_template );
		}
		if ( ! empty( $add_to_cart_gdpr_message ) ) {
			$data[ RNOC_PLUGIN_PREFIX
			       . 'add_to_cart_popup_gdpr_compliance' ][0][ RNOC_PLUGIN_PREFIX
			                                                   . 'gdpr_compliance_checkbox_message' ]
				= $this->sanitizeBasicHtml( $add_to_cart_gdpr_message );
		}
		if ( ! empty( $ei_gdpr_message ) ) {
			$data[ RNOC_PLUGIN_PREFIX
			       . 'exit_intent_popup_gdpr_compliance' ][0][ RNOC_PLUGIN_PREFIX
			                                                   . 'gdpr_compliance_checkbox_message' ]
				= $this->sanitizeBasicHtml( $ei_gdpr_message );
		}
		if ( ! empty( $coupon_mail_template ) ) {
			$data[ RNOC_PLUGIN_PREFIX
			       . 'modal_coupon_settings' ][0][ RNOC_PLUGIN_PREFIX
			                                       . 'coupon_mail_template' ]
				= $this->sanitizeBasicHtml( $coupon_mail_template );
		}
		...
		switch ( $page ) {
			default:
			case "atcp":
				if ( ! isset( $data[ RNOC_PLUGIN_PREFIX
				                     . 'modal_display_pages' ] )
				) {
					$data[ RNOC_PLUGIN_PREFIX . 'modal_display_pages' ]
						= array();
				}
				if ( ! isset( $data[ RNOC_PLUGIN_PREFIX
				                     . 'modal_hide_pages' ] )
			...
			case "ct":
				if ( ! isset( $data[ RNOC_PLUGIN_PREFIX
				                     . 'coupon_timer_display_pages' ] )
				) {
					$data[ RNOC_PLUGIN_PREFIX . 'coupon_timer_display_pages' ]
						= array();
				}
				break;
			case "eip":
				if ( ! isset( $data[ RNOC_PLUGIN_PREFIX
				                     . 'exit_intent_popup_display_pages' ] )
				) {
					$data[ RNOC_PLUGIN_PREFIX
					       . 'exit_intent_popup_display_pages' ]
						= array();
		...
		wp_send_json_success( __( 'Settings successfully saved!',
			RNOC_TEXT_DOMAIN ) );
	}
	...
	function retainfulPremiumAddOnsPage() {
		$page_slug            = $this->slug . '_premium';
		$available_addon_list = apply_filters( 'rnoc_get_premium_addon_list',
			array() );
		$base_url             = admin_url( 'admin.php?page=' . $page_slug );
		$add_on               = self::$input->get( 'add-on', null );
		if ( ! empty( $add_on ) ) {
			$settings                               = get_option( $page_slug,
				array() );
			$default_settings
			                                        = $this->getDefaultPremiumAddonsValues();
			...
			foreach ( $check_default_value_of_multi_dim_array as $key ) {
				if ( isset( $settings[ RNOC_PLUGIN_PREFIX . $key ] )
				     && isset( $settings[ RNOC_PLUGIN_PREFIX . $key ][0] )
				     && isset( $default_settings[ RNOC_PLUGIN_PREFIX . $key ] )
				     && isset( $default_settings[ RNOC_PLUGIN_PREFIX
				                                  . $key ][0] )
				     && is_array( $settings[ RNOC_PLUGIN_PREFIX . $key ][0] )
				     && is_array( $default_settings[ RNOC_PLUGIN_PREFIX
				                                     . $key ][0] )
				) {
					$settings[ RNOC_PLUGIN_PREFIX . $key ][0]
						= array_merge( $default_settings[ RNOC_PLUGIN_PREFIX
						                                  . $key ][0],
						$settings[ RNOC_PLUGIN_PREFIX . $key ][0] );
				}
			}
			...
			require_once dirname( __FILE__ )
			             . '/templates/pages/premium-addon-settings.php';
		} else {
			require_once dirname( __FILE__ )
			             . '/templates/pages/premium-addons.php';
		}
	}
	...
	function getDefaultPremiumAddonsValues() {
		$default_settings = array(
			...
			. 'coupon_timer_coupon'                                          => '',
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_display_pages'                                   => array(),
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_apply_coupon'                                    => 'automatically',
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_expire_time'                                     => '15',
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_expired_text'                                    => 'EXPIRED',
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_expire_message'                                  => __( 'Sorry! Instant Offer has expired.',
				RNOC_TEXT_DOMAIN ),
			RNOC_PLUGIN_PREFIX
			. 'auto_fix_page_reload'                                         => '0',
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_top_position_settings'                           => array(
				0 => array(
					RNOC_PLUGIN_PREFIX . 'enable_position'                 => '1',
					RNOC_PLUGIN_PREFIX
					. 'top_bottom_position'                                => 'top',
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_message'                               => __( "Make purchase quickly, your {{coupon_code}} will expire within {{coupon_timer}}",
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_display_format'                        => __( " {{minutes}}M {{seconds}}S",
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_background'                            => '#ffffff',
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_color'                                 => '#000000',
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_coupon_code_color'                     => '#000000',
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_coupon_timer_color'                    => '#000000',
					RNOC_PLUGIN_PREFIX
					. 'checkout_button_color'                              => '#ffffff',
					RNOC_PLUGIN_PREFIX
					. 'checkout_button_bg_color'                           => '#f27052',
					RNOC_PLUGIN_PREFIX
					. 'checkout_button_text'                               => __( 'Checkout Now',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX . 'enable_checkout_button'          => 1,
				)
			),
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_above_cart_position_settings'                    => array(
				0 => array(
					RNOC_PLUGIN_PREFIX . 'enable_position'                 => '1',
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_message'                               => __( "Make purchase quickly, your {{coupon_code}} will expire within {{coupon_timer}}",
			...
			RNOC_PLUGIN_PREFIX
			. 'coupon_timer_below_discount_position_settings'                => array(
				0 => array(
					RNOC_PLUGIN_PREFIX . 'enable_position'                 => '1',
					RNOC_PLUGIN_PREFIX
					. 'coupon_timer_message'                               => __( "Make purchase quickly, your {{coupon_code}} will expire within {{coupon_timer}}",
			...
			. 'need_modal'                                                   => '0',
			RNOC_PLUGIN_PREFIX
			. 'modal_email_is_mandatory'                                     => '1',
			RNOC_PLUGIN_PREFIX
			. 'modal_no_thanks_action'                                       => '1',
			RNOC_PLUGIN_PREFIX
			. 'close_btn_behavior'                                           => 'just_close',
			RNOC_PLUGIN_PREFIX
			. 'modal_show_popup_until'                                       => '1',
			RNOC_PLUGIN_PREFIX
			. 'no_conflict_mode'                                             => 'yes',
			RNOC_PLUGIN_PREFIX
			. 'modal_display_pages'                                          => array(),
			RNOC_PLUGIN_PREFIX
			. 'modal_hide_pages'                                             => array(),
			RNOC_PLUGIN_PREFIX
			. 'add_to_cart_extra_class'                                      => '',
			RNOC_PLUGIN_PREFIX
			. 'modal_design_settings'                                        => array(
				0 => array(
					RNOC_PLUGIN_PREFIX
					. 'modal_heading'                                      => __( 'Enter your email to add this item to cart',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'modal_heading_color'                                => '#000000',
					RNOC_PLUGIN_PREFIX
					. 'modal_email_placeholder'                            => __( 'Email address',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'modal_email_field_width'                            => 70,
					RNOC_PLUGIN_PREFIX
					. 'modal_add_cart_text'                                => __( 'Add to Cart',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'modal_button_field_width'                           => 70,
					RNOC_PLUGIN_PREFIX
					. 'modal_add_cart_color'                               => '#ffffff',
					RNOC_PLUGIN_PREFIX
					. 'modal_add_cart_bg_color'                            => '#f27052',
					RNOC_PLUGIN_PREFIX
					. 'modal_add_cart_border_top_color'                    => '#f27052',
					RNOC_PLUGIN_PREFIX
					. 'modal_bg_color'                                     => '#F8F0F0',
					RNOC_PLUGIN_PREFIX
					. 'modal_not_mandatory_text'                           => __( 'No thanks! Add item to cart',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'modal_add_cart_no_thanks_color'                     => '#f27052',
					RNOC_PLUGIN_PREFIX
					. 'modal_terms_text'                                   => __( '*By completing this, you are signing up to receive our emails. You can unsubscribe at any time.',
						RNOC_TEXT_DOMAIN ),
				)
			),
			RNOC_PLUGIN_PREFIX
			. 'add_to_cart_popup_gdpr_compliance'                            => array(
				0 => array(
					RNOC_PLUGIN_PREFIX
					. 'gdpr_compliance_checkbox_settings'                   => 'no_need_gdpr',
					RNOC_PLUGIN_PREFIX
					. 'gdpr_compliance_checkbox_message'                    => __( 'I accept the <a href="#">Terms and conditions</a>',
						RNOC_TEXT_DOMAIN ),
				)
			),
			RNOC_PLUGIN_PREFIX
			. 'modal_coupon_settings'                                        => array(
				0 => array(
					...
					. 'woo_coupon'                                           => '',
					RNOC_PLUGIN_PREFIX
					. 'modal_sub_heading'                                    => __( 'Get a discount in your email!',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'modal_sub_heading_color'                              => '#333333',
					RNOC_PLUGIN_PREFIX
					. 'show_woo_coupon'                                      => 'send_via_email',
					RNOC_PLUGIN_PREFIX
					. 'add_to_cart_coupon_popup_template'                    => '',
					RNOC_PLUGIN_PREFIX
					. 'coupon_mail_template_subject'                         => __( 'Your coupon code',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'coupon_mail_template'                                 => '',
				)
			...
			RNOC_PLUGIN_PREFIX
			. 'need_exit_intent_modal'                                       => 0,
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_popup_display_pages'                              => array(),
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_popup_display_to'                                 => 'all',
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_modal_coupon'                                     => '',
			RNOC_PLUGIN_PREFIX
			. 'need_exit_intent_modal_after_coupon_applied'                  => 0,
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_popup_show_settings'                              => array(
				'show_option' => 'once_per_page',
				'show_count'  => 1
			),
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_modal_cookie_life'                                => 1,
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_popup_template'                                   => '',
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_modal_custom_style'                               => '',
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_modal_redirect_on_success'                        => 'checkout',
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_popup_gdpr_compliance'                            => array(
				0 => array(
					RNOC_PLUGIN_PREFIX
					. 'gdpr_compliance_checkbox_settings'                   => 'no_need_gdpr',
					RNOC_PLUGIN_PREFIX
			...
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_popup_form_design'                                => array(
				0 => array(
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_email_placeholder'              => __( 'Enter E-mail address',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_email_height'                   => '46px',
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_email_width'                    => '100%',
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_button_text'                    => __( 'Complete checkout',
						RNOC_TEXT_DOMAIN ),
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_button_color'                   => '#ffffff',
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_button_bg_color'                => '#f20561',
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_button_height'                  => '100%',
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_form_button_width'                   => '100%',
				)
			),
			RNOC_PLUGIN_PREFIX
			. 'exit_intent_popup_mobile_settings'                            => array(
				0 => array(
					RNOC_PLUGIN_PREFIX . 'enable_mobile_support'          => '0',
					RNOC_PLUGIN_PREFIX
					. 'enable_mobile_back_click'                          => '0',
					RNOC_PLUGIN_PREFIX
					. 'enable_delay_trigger'                              => '0',
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_popup_delay_sec'                       => '0',
					RNOC_PLUGIN_PREFIX
					. 'enable_scroll_distance_trigger'                    => '0',
					RNOC_PLUGIN_PREFIX
					. 'exit_intent_modal_distance'                        => '0',
				)
			),
		);

		return apply_filters( 'rnoc_premium_addon_default_values',
			$default_settings );
	}
	...
	function saveNocSettings() {
		...
		Validator::addRule( 'basicTags',
			array( __CLASS__, 'validateBasicHtmlTags' ),
			'Only br, strong, span,div, p tags accepted' );
		$validator->rule( 'regex', RNOC_PLUGIN_PREFIX . 'expire_date_format',
			'/^[a-zA-Z0-9 ,\/:-]+$/' )
		          ->message( 'This filed should accepts number, alphabets, hypen, comma, colon and space' );
		$validator->rule( 'array', array(
		...
		$validator->rule( 'in',
			RNOC_PLUGIN_PREFIX . 'retainful_coupon_applicable_to',
			[ 'all', 'validate_on_checkout', 'login_users' ] )
		          ->message( 'This field contains invalid value' );
		$validator->rule( 'in',
			RNOC_PLUGIN_PREFIX . 'retainful_add_coupon_message_to', [
		...
		$coupon_msg = self::$input->post( RNOC_PLUGIN_PREFIX
		                                  . 'retainful_coupon_message', '',
			false );
		$applied_coupon_msg
		            = self::$input->post( RNOC_PLUGIN_PREFIX
		                                  . 'coupon_applied_popup_design', '',
			false );
		$post
		            = self::$input->post();
		$data
		            = $this->clean( $post );
		$data[ RNOC_PLUGIN_PREFIX . 'retainful_coupon_message' ]
		            = $this->sanitizeBasicHtml( $coupon_msg );
		$data[ RNOC_PLUGIN_PREFIX . 'coupon_applied_popup_design' ]
		            = $this->sanitizeBasicHtml( $applied_coupon_msg );
	...
	function getDefaultNocSettings() {
		$default_settings = array(
			...
			RNOC_PLUGIN_PREFIX
			. 'retainful_coupon_type'                                       => '0',
			RNOC_PLUGIN_PREFIX
			. 'retainful_coupon_amount'                                     => '10',
			RNOC_PLUGIN_PREFIX
			. 'retainful_expire_days'                                       => '60',
			RNOC_PLUGIN_PREFIX
			. 'expire_date_format'                                          => 'F j, Y',
			RNOC_PLUGIN_PREFIX
			. 'retainful_coupon_applicable_to'                              => 'all',
			RNOC_PLUGIN_PREFIX
			. 'automatically_generate_coupon'                               => '1',
			RNOC_PLUGIN_PREFIX
			. 'show_next_order_coupon_in_thankyou_page'                     => '0',
			RNOC_PLUGIN_PREFIX
			. 'retainful_add_coupon_message_to'                             => 'woocommerce_email_customer_details',
			RNOC_PLUGIN_PREFIX
			...
			RNOC_PLUGIN_PREFIX
			. 'preferred_order_status'                                      => array(
				'wc-processing',
				'wc-completed'
			),
			RNOC_PLUGIN_PREFIX
			. 'preferred_user_roles'                                        => array( 'all' ),
			RNOC_PLUGIN_PREFIX
			. 'limit_per_user'                                              => 99,
			RNOC_PLUGIN_PREFIX
			. 'minimum_sub_total'                                           => '',
			RNOC_PLUGIN_PREFIX
			. 'exclude_generating_coupon_for_products'                      => array(),
			RNOC_PLUGIN_PREFIX
			. 'exclude_generating_coupon_for_categories'                    => array(),
			RNOC_PLUGIN_PREFIX
			. 'minimum_spend'                                               => '',
			RNOC_PLUGIN_PREFIX
			. 'maximum_spend'                                               => '',
			...
			RNOC_PLUGIN_PREFIX
			. 'enable_coupon_applied_popup'                                 => '1',
			RNOC_PLUGIN_PREFIX
			. 'coupon_applied_popup_design'                                 => $this->appliedCouponDefaultTemplate(),
		);

		return apply_filters( 'rnoc_get_default_noc_settings',
			$default_settings );
	}
	...
	function nextOrderCouponPage() {
		...
		$apply_coupon_for     = array(
			'all'                  => __( 'Allow any one to apply coupon',
				RNOC_TEXT_DOMAIN ),
			'validate_on_checkout' => __( 'Allow the customer to apply coupon, but validate at checkout',
				RNOC_TEXT_DOMAIN ),
			'login_users'          => __( 'Allow customer to apply coupon only after login (Not Recommended)',
				RNOC_TEXT_DOMAIN )
		);
		$display_coupon_after = array(
			'woocommerce_email_order_details'    => __( 'Order details',
				RNOC_TEXT_DOMAIN ),
			'woocommerce_email_order_meta'       => __( 'Order meta',
				RNOC_TEXT_DOMAIN ),
			'woocommerce_email_customer_details' => __( 'Customer details',
				RNOC_TEXT_DOMAIN ),
			'none'                               => __( 'Do Not Show - Customers will not get next order coupon in the order confirmation email of WooCommerce',
				RNOC_TEXT_DOMAIN ),
		);
		...
		require_once dirname( __FILE__ )
		             . '/templates/pages/next-order-coupon.php';
	}
	 ...
	 * @return bool
	 */
	static function validateBasicHtmlTags(
		$field, $value, array $params, array $fields
	) {
	...
	function saveAcSettings() {
		...
		$validator->rule( 'in', RNOC_PLUGIN_PREFIX . 'cart_tracking_engine',
			[ 'js', 'php' ] )->message( 'This field contains invalid value' );
		$validator->rule( 'in', array(
		...
		$cart_capture_msg = self::$input->post( RNOC_PLUGIN_PREFIX
		                                        . 'cart_capture_msg', '' );
		$post             = self::$input->post();
		$data
		                  = $this->clean( $post );
		$data[ RNOC_PLUGIN_PREFIX . 'cart_capture_msg' ]
		                  = trim( $this->sanitizeBasicHtml( $cart_capture_msg ) );
	...
	function createWebhook() {
		if ( is_admin() ) {
			if ( $this->isConnectionActive() ) {
				...
				if ( isset( $hook_status['order.updated'] )
				     && ! $hook_status['order.updated']
				) {
					$this->addNewWebhook();
				}
				if ( isset( $hook_status['order.created'] )
				     && ! $hook_status['order.created']
				) {
	...
	function removeWebhook() {
		if ( ! class_exists( 'WC_Data_Store' )
		     || ! class_exists( '\Rnoc\Retainful\library\RetainfulApi' )
		     || ! function_exists( 'wc_get_webhook' )
		) {
			return;
		}
		try {
	...
	protected function addNewWebHook( $topic = 'order.updated' ) {
		...
		try {
			...
			$name    = $topic == 'order.updated'
				? sanitize_text_field( wp_unslash( 'Retainful Order Update' ) )
				: sanitize_text_field( wp_unslash( 'Retainful Order Create' ) );
	...
	function retainfulSettingsPage() {
		...
		$default_settings = array(
			...
			RNOC_PLUGIN_PREFIX
			. 'enable_background_order_sync'                              => 'no',
			RNOC_PLUGIN_PREFIX
			. 'track_zero_value_carts'                                    => 'no',
			RNOC_PLUGIN_PREFIX
			. 'enable_referral_widget'                                    => 'no',
			RNOC_PLUGIN_PREFIX
			. 'enable_dynamic_popup'                                      => 'yes',
			RNOC_PLUGIN_PREFIX
			. 'enable_embeded_referral_widget'                            => 'no',
			RNOC_PLUGIN_PREFIX
			...
			RNOC_PLUGIN_PREFIX
			. 'refresh_fragments_on_page_load'                            => '0',
			RNOC_PLUGIN_PREFIX
			. 'enable_gdpr_compliance'                                    => '0',
			RNOC_PLUGIN_PREFIX
			. 'cart_capture_msg'                                          => 'Keep me up to date on news and exclusive offers',
			RNOC_PLUGIN_PREFIX
			. 'gdpr_display_position'                                     => 'after_billing_email',
			RNOC_PLUGIN_PREFIX
			. 'enable_ip_filter'                                          => '0',
			...
			. 'enable_debug_log'                                          => '0',
			RNOC_PLUGIN_PREFIX
			. 'handle_storage_using'                                      => 'woocommerce',
			RNOC_PLUGIN_PREFIX
			. 'enable_afterpay_action'                                    => 'no',
			RNOC_PLUGIN_PREFIX
			. 'varnish_check'                                             => 'no',
		);
		$settings         = wp_parse_args( $settings, $default_settings );
		...
		if ( empty( $settings[ RNOC_PLUGIN_PREFIX . 'cart_capture_msg' ] ) ) {
			$settings[ RNOC_PLUGIN_PREFIX . 'cart_capture_msg' ]
	...
	function getRetainfulSettingValue( $key, $default = null ) {
		...
		return ( ! empty( $key ) && isset( $settings[ $key ] ) )
	...
	function registerMenu() {
		add_menu_page( 'Retainful', 'Retainful', 'manage_woocommerce',
			'retainful_license', array( $this, 'retainfulLicensePage' ),
			'dashicons-controls-repeat', 56 );
		add_submenu_page( 'retainful_license', 'Connection', 'Connection',
			'manage_woocommerce', 'retainful_license',
			array( $this, 'retainfulLicensePage' ) );
		add_submenu_page( 'retainful_license', 'Settings', 'Settings',
			'manage_woocommerce', 'retainful_settings',
			array( $this, 'retainfulSettingsPage' ) );
		$settings = $this->getAdminSettings();

		$is_next_order_disable = get_option( 'retainful_hide_next_order_coupon',
			'no' );
		if ( ! $this->isNextOrderCouponEnabled() ) {
			if ( ( $is_next_order_disable === 'no'
			       || empty( $is_next_order_disable ) )
			     && ( empty( $settings ) || count( $settings ) < 3 )
			) {
		...
		$is_premium_feature_disable
		                       = get_option( 'retainful_hide_premium_feature',
			'no' );
		if ( ( $is_premium_feature_disable === 'no'
		       || empty( $is_premium_feature_disable ) )
		     && empty( $permium_page_settings )
		) {
			update_option( 'retainful_hide_premium_feature', 'yes' );
		}

		$can_hide_next_order_coupon
			= get_option( 'retainful_hide_next_order_coupon', 'no' );
		if ( $can_hide_next_order_coupon !== 'yes' ) {
			add_submenu_page( 'retainful_license', 'Settings',
				'Next order coupon', 'manage_woocommerce', 'retainful',
				array( $this, 'nextOrderCouponPage' ) );
		}

		$can_hide_premium_feature
			= get_option( 'retainful_hide_premium_feature', 'no' );
		if ( $can_hide_premium_feature !== 'yes' ) {
			add_submenu_page( 'retainful_license', 'Settings',
				'Premium features', 'manage_woocommerce', 'retainful_premium',
				array( $this, 'retainfulPremiumAddOnsPage' ) );
		}
		...
		if ( isset( $_REQUEST['page'] )
			...
			add_action( 'admin_notices', function () use ( $legacy_notice ) {
				echo '<div class="error notice"><p>' . $legacy_notice
				     . '</p></div>';
			} );
		}
		...
		if ( isset( $_REQUEST['page'] )
		     ...
		     && $this->isWebhookNoticeShow()
		) {
			$message
				= sprintf( __( 'Webhooks for Retainful seem not present or de-activated. Please go to the WooCommerce <a href="%s" target="_blank">webhooks section</a> and activate them.',
				RNOC_TEXT_DOMAIN ),
	...
	function isWebhookNoticeShow() {
		...
		try {
			...
			foreach ( $webhooks as $webhook_id ) {
				...
				$delivery_url      = $webhook->get_delivery_url();
				$site_delivery_url = $this->api->getDomain()
				                     . 'woocommerce/webhooks/checkout';
				if ( $delivery_url != $site_delivery_url ) {
					continue;
				}
	...
	function unScheduleHooks() {
		$this->removeFinishedHooks( 'rnoc_abandoned_clear_abandoned_carts',
			'pending' );
		$this->removeFinishedHooks( 'rnoc_abandoned_cart_send_email',
			'pending' );
	}
	...
	function schedulePlanChecker() {
		$this->scheduleEvents( 'rnocp_check_user_plan',
			current_time( 'timestamp' ), array(), 'recurring', 604800 );
	}
	...
	function scheduleEvents(
		$hook, $timestamp, $args = array(), $type = "single",
		$interval_in_seconds = null, $group = ''
	) {
				...
				case "recurring":
					if ( ! $this->nextScheduledAction( $hook ) ) {
						\ActionScheduler::factory()
						                ->recurring( $hook, $args, $timestamp,
							                $interval_in_seconds, $group );
					}
					break;
				case 'single':
				default:
					$action_id = \ActionScheduler::factory()
					                             ->single( $hook, $args,
						                             $timestamp );
			...
			switch ( $type ) {
				case "recurring":
					if ( function_exists( 'as_schedule_recurring_action' )
						...
						if ( ! as_next_scheduled_action( $hook ) ) {
							as_schedule_recurring_action( $timestamp,
								$interval_in_seconds, $hook, $args, $group );
						}
					}
					break;
				case 'single':
				default:
					if ( function_exists( 'as_schedule_single_action' ) ) {
						$action_id = as_schedule_single_action( $timestamp,
							$hook, $args );
						$this->addPostMeta( $action_id, $args );
					}
					break;
	...
	function nextScheduledAction( $hook, $args = null, $group = '' ) {
		...
		if ( defined( 'WC_VERSION' )
			...
			$job_id           = \ActionScheduler::store()
			                                    ->find_action( $hook, $params );
			if ( ! empty( $job_id ) ) {
				return true;
			}
	...
	protected function availableScheduledActions() {
		return array(
			'rnocp_check_user_plan',
			'rnoc_abandoned_clear_abandoned_carts',
			'rnoc_abandoned_cart_send_email'
		);
	}
	...
	function removeFinishedHooks( $post_title, $status = "" ) {
		try {
			...
			if ( ! empty( $post_title )
			     && ! in_array( $post_title, $available_action_names )
			) {
				return false;
			}
			global $wpdb;
			//when post table is using by scheduler
			$post_where = ( ! empty( $status ) )
				? "AND post_status = '{$status}'" : "";
			$scheduled_actions
			            = $wpdb->get_results( "SELECT ID from `{$wpdb->prefix}posts` where post_title ='{$post_title}' {$post_where} AND  post_type='scheduled-action' LIMIT 500" );
			if ( ! empty( $scheduled_actions ) ) {
				foreach ( $scheduled_actions as $action ) {
					if ( wp_delete_post( $action->ID, true ) ) {
						do_action( 'action_scheduler_deleted_action',
							$action->ID );
					}
				}
			}
			...
			if ( $wpdb->get_var( $query ) == $custom_table_name ) {
				$custom_table_where = ( ! empty( $status ) )
					? "AND status = '{$status}'" : "";
				$scheduled_actions
				                    = $wpdb->get_results( "SELECT action_id from `{$custom_table_name}` where hook ='{$post_title}' {$custom_table_where} LIMIT 500" );
				if ( ! empty( $scheduled_actions ) ) {
					foreach ( $scheduled_actions as $action ) {
						$deleted = $wpdb->delete( $custom_table_name,
	...
	function logMessage( $message, $log_in_as = "checkout" ) {
		...
		if ( isset( $admin_settings[ RNOC_PLUGIN_PREFIX . 'enable_debug_log' ] )
		     && ! empty( $admin_settings[ RNOC_PLUGIN_PREFIX
		                                  . 'enable_debug_log' ] )
		     && ! empty( $message )
		) {
			try {
	...
	function getStorageHandler() {
		$admin_settings = $this->getAdminSettings();
		if ( isset( $admin_settings[ RNOC_PLUGIN_PREFIX
		                             . 'handle_storage_using' ] )
		     && ! empty( $admin_settings[ RNOC_PLUGIN_PREFIX
		                                  . 'handle_storage_using' ] )
		) {
			return $admin_settings[ RNOC_PLUGIN_PREFIX
			                        . 'handle_storage_using' ];
		} else {
			return "woocommerce";
		}
	...
	function isInstalledFresh() {
		...
		$required_tables = array(
			$wpdb->prefix . RNOC_PLUGIN_PREFIX . 'abandoned_cart_history',
			$wpdb->prefix . RNOC_PLUGIN_PREFIX . 'email_templates'
		);
		if ( ! empty( $tables_list ) ) {
			foreach ( $tables_list as $table_name ) {
				if ( count( array_intersect( $required_tables, $table_name ) )
				     > 0
				) {
					return 0;
				}
	...
	function getUserActivePlan() {
		...
		return strtolower( trim( isset( $plan_details['plan'] )
	...
	function getUserPlanStatus() {
		...
		return strtolower( trim( isset( $plan_details['status'] )
			? $plan_details['status'] : 'inactive' ) );
	}
	...
	function getCartTrackingEngine() {
		...
		return ( isset( $settings[ RNOC_PLUGIN_PREFIX
		                           . 'cart_tracking_engine' ] )
		         && ! empty( $settings[ RNOC_PLUGIN_PREFIX
		                                . 'cart_tracking_engine' ] ) )
			? $settings[ RNOC_PLUGIN_PREFIX . 'cart_tracking_engine' ] : 'js';
	}
	...
	function needReferralWidget() {
		$settings    = $this->getAdminSettings();
		$need_widget = ( isset( $settings[ RNOC_PLUGIN_PREFIX
		                                   . 'enable_referral_widget' ] )
		                 && ! empty( $settings[ RNOC_PLUGIN_PREFIX
		                                        . 'enable_referral_widget' ] ) )
			? $settings[ RNOC_PLUGIN_PREFIX . 'enable_referral_widget' ] : 'no';

		return apply_filters( "retainful_enable_referral_program",
			( $need_widget === "yes" ) );
	}
	...
	function needPopupWidget() {
		$settings    = $this->getAdminSettings();
		$need_widget = ( isset( $settings[ RNOC_PLUGIN_PREFIX
		                                   . 'enable_dynamic_popup' ] )
		                 && ! empty( $settings[ RNOC_PLUGIN_PREFIX
		                                        . 'enable_dynamic_popup' ] ) )
			? $settings[ RNOC_PLUGIN_PREFIX . 'enable_dynamic_popup' ] : 'no';

		return apply_filters( "retainful_enable_popup_widget",
			( $need_widget === "yes" ) );
	}
	...
	function needEmbededReferralWidget() {
		$settings    = $this->getAdminSettings();
		$need_widget = ( isset( $settings[ RNOC_PLUGIN_PREFIX
		                                   . 'enable_embeded_referral_widget' ] )
		                 && ! empty( $settings[ RNOC_PLUGIN_PREFIX
		                                        . 'enable_embeded_referral_widget' ] ) )
			? $settings[ RNOC_PLUGIN_PREFIX . 'enable_embeded_referral_widget' ]
			: 'yes';

		return apply_filters( "enable_embeded_referral_widget",
			( $need_widget === "yes" ) );
	}

	function isAfterPayEnabled() {
		$settings      = $this->getAdminSettings();
		$need_afterpay = ( isset( $settings[ RNOC_PLUGIN_PREFIX
		                                     . 'enable_afterpay_action' ] )
		                   && ! empty( $settings[ RNOC_PLUGIN_PREFIX
		                                          . 'enable_afterpay_action' ] ) )
			? $settings[ RNOC_PLUGIN_PREFIX . 'enable_afterpay_action' ] : 'no';

		return apply_filters( "retainful_enable_afterpay_action",
			( $need_afterpay === "yes" ) );
	}
	...
	function trackZeroValueCarts() {
		...
		return ( isset( $settings[ RNOC_PLUGIN_PREFIX
		                           . 'track_zero_value_carts' ] )
		         && ! empty( $settings[ RNOC_PLUGIN_PREFIX
		                                . 'track_zero_value_carts' ] ) )
			? $settings[ RNOC_PLUGIN_PREFIX . 'track_zero_value_carts' ] : 'no';
	}
	...
	function getEmailTemplatesSettings() {
		$abandoned_cart_email_templates = get_option( $this->slug
		                                              . '_abandoned_cart_email_templates',
			array() );
	...
	function getCouponExpireDate( $ordered_date ) {
		...
		$settings    = get_option( $this->slug, array() );
		$expire_days = isset( $settings[ RNOC_PLUGIN_PREFIX
		                                 . 'retainful_expire_days' ] )
			? intval( $settings[ RNOC_PLUGIN_PREFIX
			                     . 'retainful_expire_days' ] ) : 0;
		if ( $this->isAppConnected() && $expire_days > 0 ) {
			$response['retainful_coupons']
				                     = $this->addDaysToDate( $ordered_date,
				$expire_days );
			$response['woo_coupons'] = $this->addDaysToDate( $ordered_date,
				$expire_days + 1 );
		}
	...
	function addScript() {
		...
		wp_enqueue_script( 'rnoc-select2-js',
			$this->getWooPluginUrl() . '/assets/js/select2/select2.full.min.js',
			array( 'jquery' ) );
		wp_enqueue_style( 'rnoc-select2-css',
			$this->getWooPluginUrl() . '/assets/css/select2.css' );
		...
		wp_enqueue_style( 'retainful-admin-css', $asset_path . '/css/main.css',
			array(), RNOC_VERSION );
		wp_enqueue_style( 'retainful-admin-style-css',
			$asset_path . '/css/style.css', array(), RNOC_VERSION );
	...
	function getExpireDateFormat() {
		...
		if ( isset( $usage_restriction[ RNOC_PLUGIN_PREFIX
		                                . 'expire_date_format' ] )
		     && ! empty( $usage_restriction[ RNOC_PLUGIN_PREFIX
		                                     . 'expire_date_format' ] )
		) {
			return $usage_restriction[ RNOC_PLUGIN_PREFIX
			                           . 'expire_date_format' ];
		}
	...
	function getUserRoles() {
		...
		if ( ! empty( $all_roles ) ) {
			foreach ( $all_roles as $role_name => $role ) {
				$user_roles[ $role_name ] = isset( $role['name'] )
					? $role['name'] : '';
			}
		}
	...
	function getPlanDetails() {
		...
		if ( empty( $plan_details ) ) {
			...
			if ( ! empty( $api_key ) ) {
				$api_obj    = new RestApi();
				$store_data = array(
					'secret_key' => $api_obj->encryptData( $api_key,
						$secret_key )
				);
	...
	function isProPlan() {
		...
		return ( in_array( $plan,
				array( 'pro', 'business', 'professional', 'essential' ) )
		         && in_array( $status, array( 'active', 'trialing' ) ) );
	}
	...
	function unlockPremiumLink() {
		return '<a href="' . $this->api->upgradePremiumUrl() . '">'
		       . __( "Unlock this feature by upgrading to Premium",
				RNOC_TEXT_DOMAIN ) . '</a>';
	}
	...
	function isConnectionActive() {
		...
		if ( $this->isAppConnected() && ! empty( $secret_key )
		     && ! empty( $app_id )
		) {
			return true;
		}
	...
	function isAppConnected() {
		...
		if ( ! empty( $settings )
		     && isset( $settings[ RNOC_PLUGIN_PREFIX
		                          . 'is_retainful_connected' ] )
		     && ! empty( $settings[ RNOC_PLUGIN_PREFIX
		                            . 'is_retainful_connected' ] )
		) {
			return true;
	...
	function getApiKey() {
		$settings = $this->getLicenseDetails();
		if ( ! empty( $settings )
		     && isset( $settings[ RNOC_PLUGIN_PREFIX . 'retainful_app_id' ] )
		     && ! empty( $settings[ RNOC_PLUGIN_PREFIX . 'retainful_app_id' ] )
		) {
	...
	function getBaseCurrency() {
		...
		return apply_filters( 'rnoc_get_default_currency_code',
			$base_currency );
	}
	...
	function getCouponMessage() {
		...
		if ( ! empty( $settings )
		     && isset( $settings[ RNOC_PLUGIN_PREFIX
		                          . 'retainful_coupon_message' ] )
		     && ! empty( isset( $settings[ RNOC_PLUGIN_PREFIX
		                                   . 'retainful_coupon_message' ] ) )
		) {
			return __( $settings[ RNOC_PLUGIN_PREFIX
			                      . 'retainful_coupon_message' ],
				RNOC_TEXT_DOMAIN );
		} else {
	...
	function getCouponSettings() {
		...
		$settings = get_option( $this->slug, array() );
		if ( ! empty( $settings ) ) {
			$coupon['coupon_type'] = isset( $settings[ RNOC_PLUGIN_PREFIX
			                                           . 'retainful_coupon_type' ] )
				? $settings[ RNOC_PLUGIN_PREFIX . 'retainful_coupon_type' ] : 0;
			$coupon['coupon_applicable_to']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                                           . 'retainful_coupon_applicable_to' ] )
			                         && ! empty( $settings[ RNOC_PLUGIN_PREFIX
			                                                . 'retainful_coupon_applicable_to' ] )
				? $settings[ RNOC_PLUGIN_PREFIX
				             . 'retainful_coupon_applicable_to' ] : 'all';
			$coupon['coupon_amount']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                                           . 'retainful_coupon_amount' ] )
			                         && ( $settings[ RNOC_PLUGIN_PREFIX
			                                         . 'retainful_coupon_amount' ]
			                              > 0 ) ? $settings[ RNOC_PLUGIN_PREFIX
			                                                 . 'retainful_coupon_amount' ]
				: 0;
			$coupon['product_ids']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                         ...
			                         && ! empty( $settings[ RNOC_PLUGIN_PREFIX
			                                                . 'products' ] )
				? $settings[ RNOC_PLUGIN_PREFIX . 'products' ] : array();
			$coupon['exclude_product_ids']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                                           . 'exclude_products' ] )
			                         && ! empty( $settings[ RNOC_PLUGIN_PREFIX
			                                                . 'exclude_products' ] )
				? $settings[ RNOC_PLUGIN_PREFIX . 'exclude_products' ]
				: array();
			$coupon['minimum_amount']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                         ...
			                         && ( $settings[ RNOC_PLUGIN_PREFIX
			                                         . 'minimum_spend' ] > 0 )
				? $settings[ RNOC_PLUGIN_PREFIX . 'minimum_spend' ] : 0;
			$coupon['maximum_amount']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                                           . 'maximum_spend' ] )
			                         && ( $settings[ RNOC_PLUGIN_PREFIX
			                                         . 'maximum_spend' ] > 0 )
				? $settings[ RNOC_PLUGIN_PREFIX . 'maximum_spend' ] : 0;
			...
			$coupon['product_categories']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                         ...
			                         && ! empty( $settings[ RNOC_PLUGIN_PREFIX
			                                                . 'product_categories' ] )
				? $settings[ RNOC_PLUGIN_PREFIX . 'product_categories' ]
				: array();
			$coupon['exclude_product_categories']
			                       = isset( $settings[ RNOC_PLUGIN_PREFIX
			                         ...
			                         && ! empty( $settings[ RNOC_PLUGIN_PREFIX
			                                                . 'exclude_product_categories' ] )
				? $settings[ RNOC_PLUGIN_PREFIX . 'exclude_product_categories' ]
				: array();
		}
	...
	function getCouponValidOrderStatuses() {
		...
		if ( ! empty( $settings ) ) {
			return isset( $settings[ RNOC_PLUGIN_PREFIX
			                         . 'preferred_order_status' ] )
				? $settings[ RNOC_PLUGIN_PREFIX . 'preferred_order_status' ]
				: $statuses;
		}
	...
	function showCouponInThankYouPage() {
		...
		if ( ! empty( $settings ) ) {
			return isset( $settings[ RNOC_PLUGIN_PREFIX
			                         . 'show_next_order_coupon_in_thankyou_page' ] )
				? $settings[ RNOC_PLUGIN_PREFIX
				             . 'show_next_order_coupon_in_thankyou_page' ]
				: $show_on_thankyou_page;
		}
	...
	function enableCouponResponsePopup() {
		...
		if ( ! empty( $settings ) ) {
			return isset( $settings[ RNOC_PLUGIN_PREFIX
			                         . 'enable_coupon_applied_popup' ] )
				? $settings[ RNOC_PLUGIN_PREFIX
				             . 'enable_coupon_applied_popup' ] : $enable;
		}
	...
	function getCouponValidUserRoles() {
		...
		if ( $this->isProPlan() ) {
			$usage_restrictions = get_option( $this->slug, array() );
			if ( ! empty( $usage_restrictions ) ) {
				return isset( $usage_restrictions[ RNOC_PLUGIN_PREFIX
				                                   . 'preferred_user_roles' ] )
					? $usage_restrictions[ RNOC_PLUGIN_PREFIX
					                       . 'preferred_user_roles' ] : $roles;
			}
		}
	...
	function getCouponLimitPerUser() {
		...
		if ( $this->isProPlan() ) {
			...
			if ( ! empty( $settings ) ) {
				return isset( $settings[ RNOC_PLUGIN_PREFIX
				                         . 'limit_per_user' ] )
					? $settings[ RNOC_PLUGIN_PREFIX . 'limit_per_user' ]
					: $limit;
			}
		}
	...
	function getMinimumOrderTotalForCouponGeneration() {
		...
		if ( $this->isProPlan() ) {
			...
			if ( ! empty( $settings ) ) {
				return isset( $settings[ RNOC_PLUGIN_PREFIX
				                         . 'minimum_sub_total' ] )
					? $settings[ RNOC_PLUGIN_PREFIX . 'minimum_sub_total' ]
					: $minimum_sub_total;
			}
		}
	...
	function getInvalidProductsForCoupon() {
		...
		if ( $this->isProPlan() ) {
			...
			if ( ! empty( $settings ) ) {
				return isset( $settings[ RNOC_PLUGIN_PREFIX
				                         . 'exclude_generating_coupon_for_products' ] )
					? $settings[ RNOC_PLUGIN_PREFIX
					             . 'exclude_generating_coupon_for_products' ]
					: $products;
			}
	...
	function getInvalidCategoriesForCoupon() {
		...
		if ( $this->isProPlan() ) {
			...
			if ( ! empty( $settings ) ) {
				return isset( $settings[ RNOC_PLUGIN_PREFIX
				                         . 'exclude_generating_coupon_for_categories' ] )
					? $settings[ RNOC_PLUGIN_PREFIX
					             . 'exclude_generating_coupon_for_categories' ]
					: $categories;
			}
	...
	function autoGenerateCouponsForOldOrders() {
		$settings = get_option( $this->slug, array() );
		if ( ! empty( $settings ) ) {
			if ( isset( $settings[ RNOC_PLUGIN_PREFIX
			                       . 'automatically_generate_coupon' ] )
			     && $settings[ RNOC_PLUGIN_PREFIX
			                   . 'automatically_generate_coupon' ] == 0
			) {
				return false;
			}
		}
	...
	function couponFor() {
		...
		$settings              = get_option( $this->slug, array() );
		if ( ! empty( $settings ) ) {
			$coupon_applicable_for = ( $settings[ RNOC_PLUGIN_PREFIX
			                                      . 'retainful_coupon_applicable_to' ] )
				? $settings[ RNOC_PLUGIN_PREFIX
				             . 'retainful_coupon_applicable_to' ] : 'all';
		}
	...
	function couponMessageHook() {
		...
		if ( ! empty( $settings ) && is_array( $settings ) ) {
			$hook = ( isset( $settings[ RNOC_PLUGIN_PREFIX
			                            . 'retainful_add_coupon_message_to' ] )
			          && $settings[ RNOC_PLUGIN_PREFIX
			                        . 'retainful_add_coupon_message_to' ] )
				? $settings[ RNOC_PLUGIN_PREFIX
				             . 'retainful_add_coupon_message_to' ]
	...
	function setupSurveyForm() {
		if ( ! apply_filters( 'rnoc_need_survey_form', true ) ) {
			return false;
		}
		$survey = new Survey();
		$survey->init( RNOC_PLUGIN_SLUG,
			'Retainful - next order coupon for woocommerce', RNOC_TEXT_DOMAIN );
	}
	...
	function isBackgroundOrderSyncEnabled() {
		...
		return isset( $settings[ RNOC_PLUGIN_PREFIX
		                         . 'enable_background_order_sync' ] )
		       && $settings[ RNOC_PLUGIN_PREFIX
		                     . 'enable_background_order_sync' ] == 'yes';
	}
	...
	function setIdentity( $value = '' ) {
		if ( ! $this->isCustomerPage() || empty( $value )
		     || ! $this->needPopupWidget()
		) {
			return;
		}
		$cookie      = new Cookie();
		...
		if ( function_exists( 'wc_setcookie' ) ) {
			wc_setcookie( '_wc_rnoc_tk_session',
	...
	function setIdentityData() {
		$customer_billing_email
			= $this->wc_functions->getCustomerBillingEmail();
		if ( $this->isCustomerPage() && empty( $customer_billing_email ) ) {
			...
			if ( ! empty( $cookie_email ) ) {
				$cookie_email = json_decode( base64_decode( $cookie_email ),
					true );
				if ( is_array( $cookie_email )
				     && ! empty( $cookie_email['email'] )
				)
