...
class RetainfulApi
{
     ...
     * @return array|bool|mixed|object|string
     */
    function retrieveCartDetailsWithCartId($app_id, $cart_id, $cart_hash, $ip)
    {
        $url = rtrim($this->domain, '/');
        $url .= '/webhook/woocommerce/cart';
        $request_url = add_query_arg(array('app_id' => $app_id, 'rnoc_cart' => $cart_id, 'rnoc_hash' => $cart_hash), $url);
        $headers = array(
            'app_id' => $app_id,
