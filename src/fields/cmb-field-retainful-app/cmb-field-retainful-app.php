...
class CMB2_Field_Retainful_App
{
    ...
    public function render_retainful_app($field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object)
    {
        ...
        <img src="<?= $asset_path ?>/images/loader.gif" width="20px;" id="connect-to-retainful-loader" style="display: none;" />
        <?php
        if ($is_app_connected) {
            ?>
            <button type="button" id="disconnect-app-btn" class="button"><?= __('Dis-connect', RNOC_TEXT_DOMAIN) ?></button>
            <?php
        }
        ?>
