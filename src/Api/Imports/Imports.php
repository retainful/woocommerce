...
class Imports extends Order {
	...
	protected function getOrders( $params ) {
		...
		if ( self::isHPOSEnabled() ) {
          ...
          WHERE type = %s AND id > %s AND status != %s AND {$wpdb->prefix}woocommerce_order_items.order_id > 0 AND {$wpdb->prefix}woocommerce_order_items.order_item_type = %s GROUP BY id ORDER BY id ASC LIMIT %d", array(
				'shop_order',
				(int) $params['since_id'],
				'trash',
				'line_item',
          ...
          WHERE post_type IN ('shop_order') AND ID > %d AND post_status != %s AND {$wpdb->prefix}woocommerce_order_items.order_id > 0 AND {$wpdb->prefix}woocommerce_order_items.order_item_type = %s GROUP BY ID ORDER BY ID ASC LIMIT %d", array(
				(int) $params['since_id'],
				'trash',
				'line_item',
				(int) $params['limit']
	...
	protected function getOrderCount() {
		...
		if ( self::isHPOSEnabled() ) {
          ...
          WHERE type = %s AND id > 0 AND status != %s AND {$wpdb->prefix}woocommerce_order_items.order_id > 0 AND {$wpdb->prefix}woocommerce_order_items.order_item_type = %s", array(
				'shop_order',
				'trash',
				'line_item'
			) );
          ...
          WHERE post_type = %s AND ID > 0 AND post_status != %s AND {$wpdb->prefix}woocommerce_order_items.order_id > 0 AND {$wpdb->prefix}woocommerce_order_items.order_item_type = %s", array(
				'shop_order',
				'trash',
				'line_item'
			) );
