...
class Settings
{
    ...
    function renderPage()
    {
        add_action('cmb2_admin_init', function () {
            ...
            $general_settings = new_cmb2_box(array(
                ...
                'title' => __('Retainful Next Order Coupon', 'retainful-coupon'),
                'object_types' => array('options-page'),
                'option_key' => $this->slug,
                'tab_group' => $this->slug,
                'parent_slug' => 'woocommerce',
                'tab_title' => __('Next order coupon', 'retainful-coupon'),
                'save_button' => __('Save', 'retainful-coupon')
            ));
            $general_settings->add_field(array(
                'name' => __('Retainful App ID', 'retainful-coupon'),
                'id' => $this->app_prefix . 'retainful_app_id',
                'type' => 'retainful_app',
                'default' => '',
                'desc' => __('You can get your App-id from https://www.app.retainful.com', 'retainful-coupon')
            ));
            $general_settings->add_field(array(
            ...
            $general_settings->add_field(array(
                'name' => __('Coupon type', 'retainful-coupon'),
                'id' => $this->app_prefix . 'retainful_coupon_type',
                'type' => 'radio',
                'classes' => 'retainful-coupon-group',
                'options' => array(
                    '0' => __('Percentage', 'retainful-coupon'),
                    '1' => __('Flat', 'retainful-coupon')
                ),
                'default' => '0'
            ));
            $general_settings->add_field(array(
                'name' => __('Coupon value', 'retainful-coupon'),
            ...
            $general_settings->add_field(array(
                'name' => __('Apply coupon to', 'retainful-coupon'),
                'id' => $this->app_prefix . 'retainful_coupon_applicable_to',
                'type' => 'radio',
                'classes' => 'retainful-coupon-group',
                'options' => array(
                    'all' => __('Allow any one to apply coupon', 'retainful-coupon'),
                    'validate_on_checkout' => __('Allow the customer to apply coupon, but validate at checkout', 'retainful-coupon'),
                    'login_users' => __('Allow customer to apply coupon only after login, but validate at checkout', 'retainful-coupon')
                ),
                'default' => 'all'
            ));
            $general_settings->add_field(array(
                'name' => __('Display coupon message after', 'retainful-coupon'),
                'id' => $this->app_prefix . 'retainful_add_coupon_message_to',
                'type' => 'select',
                'classes' => 'retainful-coupon-group',
                'options' => array(
                    /*'woocommerce_email_header' => __('Header', 'retainful-coupon'),*/
                    'woocommerce_email_order_details' => __('Order details', 'retainful-coupon'),
                    'woocommerce_email_order_meta' => __('Order meta', 'retainful-coupon'),
                    'woocommerce_email_customer_details' => __('Customer details', 'retainful-coupon'),
                    /*'woocommerce_email_footer' => __('Footer', 'retainful-coupon')*/
                ),
                'default' => 'woocommerce_email_customer_details'
            ));
            $general_settings->add_field(array(
                'name' => __('Custom coupon message', 'retainful-coupon'),
                ...
                'desc' => __('This message will attached to the Order Email.<br>Please use the below short codes to show the Coupon details in the message.<br><b>{{coupon_code}}</b> - Coupon code<br><b>{{coupon_amount}}</b> - Coupon amount<br><br><br>', 'retainful-coupon')
            ));
        });
    }
    ...
    function getCouponMessage()
    {
        ...
        if (!empty($settings) && isset($settings[$this->app_prefix . 'retainful_coupon_message']) && !empty(isset($settings[$this->app_prefix . 'retainful_coupon_message']))) {
            return __($settings[$this->app_prefix . 'retainful_coupon_message'], 'retainful-coupon');
        } else {
            return __('<div style="text-align: center;"><div class="coupon-block"><h3 style="font-size: 25px; font-weight: 500; color: #222; margin: 0 0 15px;">{{coupon_amount}} Off On Your Next Purchase</h3><p style="font-size: 16px; font-weight: 500; color: #555; line-height: 1.6; margin: 15px 0 20px;">To thank you for being a loyal customer we want to offer you an exclusive voucher for {{coupon_amount}} off your next order!</p><p style="text-align: center;"><span style="line-height: 1.6; font-size: 18px; font-weight: 500; background: #ffffff; padding: 10px 20px; border: 2px dashed #8D71DB; color: #8d71db; text-decoration: none;">{{coupon_code}}</span></p></div></div>', 'retainful-coupon');
        }
    }
